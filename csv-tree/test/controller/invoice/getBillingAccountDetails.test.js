...
describe('get Billing details', () => {
  ...
  it('should return a billing details reponse for discounted items', () => {
    ariaStub.getInvoiceData.resolves(testData.dataWithDiscount);
    const value = getBillingAccountDetails(config, args);
    value.then((result) => {
      result.should.be.deep.equal(testData.responseDataWithDiscount);
    });
  });
